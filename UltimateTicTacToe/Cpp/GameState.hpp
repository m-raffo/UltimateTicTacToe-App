...
const std::bitset<20> winningPosX[] = {
    0b00000000000001010100, 0b00000001010100000000, 0b01010100000000000000,
    0b00000100000100000100, 0b00010000010000010000, 0b01000001000001000000,
    0b01000000010000000100, 0b00000100010001000000,
};

const std::bitset<20> winningPosO[] = {
    0b00000000000010101000, 0b00000010101000000000, 0b10101000000000000000,
    0b00001000001000001000, 0b00100000100000100000, 0b10000010000010000000,
    0b10000000100000001000, 0b00001000100010000000,
};
...
/**
 * Evaluates the give miniboard to check for a win, including the possibility
 * that a square is tied. This is necessary for checking for wins on the larger,
 * overall board. 0: Ongoing game 1: X win 2: O win 3: Tie
 */
...
/**
 * Gets result if miniboard has been marked with final result. Will not check
 * for wins for the benefit of computation speed. 0: Ongoing game 1: X wins 2: O
 * wins 3: Tie
 */
...
class GameState {
 private:
  /**
   * Info - stores information about the GameState that is not stored on the
   * board Bits: 0-3: Required board to move on 4: Is there a required board
   * (1=yes, 0=no) 5: Player to move (1=X, 0=O)
   */
 ...
 public:
   ...
   * @param boardLocation The board (from 0 to 8) to get
   * @param pieceLocation The piece (fromr 0 to 8) to get
   * @return 0 if the position is empty, 1 if the position is claimed by X, and
   * 2 if the position is claimed by O
   */
  int getPosition(int boardLocation, int pieceLocation);

  /**
   * Performs the specificed move on the board, moving the piece whose turn it
   * is, and flipping toMove.
   */
  void move(int boardLocation, int pieceLocation);

  /**
   * Updates the game statuses of all the miniboards, checking to see if any of
   * them are won. If a position is won for both O and X and not already marked,
   * it will be marked as a win for X.
   */
   ...
   * Important: GameState.updateMiniboardStatus() MUST be called before this
   * function to ensure correct results.
   *
   * 0: Ongoing game
   * 1: X wins
   * 2: O wins
  ...
  /**
   * Converts the current board to a human-readable string representation and
   * writes it to cout.
   */
  void displayGame();
