...
CGameState *CreateGameState() { return new GameState(); }
...
int GetBestAction(CMCTS *mcts) { return mcts->maxActionProb(); }

void TakeActionMCTS(CMCTS *mcts, int action) { mcts->takeAction(action); }

void ReleaseGameState(CGameState *c) { delete c; }

void ReleaseMCTS(CMCTS *mcts) { delete mcts; }

int IsEvalNeeded(CMCTS *mcts) { return (int)mcts->evaluationNeeded; }

int GetPosition(CGameState *c, int board, int piece) {
  return c->getPosition(board, piece);
}

void SetPosition(CGameState *c, int boardLocation, int pieceLocation,
                 int piece) {
...
int GetRequiredBoard(CGameState *c) { return c->getRequiredBoard(); }
...
CboardCoords MinimaxSearchTimeMove(CGameState *position, int time,
                                   int playAsX) {
  CboardCoords result;

  boardCoords firstResult =
      minimaxSearchTimeMove(*position, time, playAsX == 1);
...
int GetToMove(CGameState *c) { return c->getToMove(); }
