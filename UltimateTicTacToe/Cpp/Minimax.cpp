 ...
 * @return The evaluation, Positive indicates advantage to X, negative indicates
 * advantage to O
 */
float evaluate(GameState board, constants c) {
  ...
  for (int i = 0; i < 9; i++) {
    finalEval += (miniboardEvalsX[i] * sigs.sigsX[i]) -
                 (miniboardEvalsO[i] * sigs.sigsO[i]);
  }
...
float miniboardEvalOneSide(bitset<20> miniboard, int side, constants c) {
  ...
  // The amount to move from location to get to the corresponding bit for the
  // other side
  ...
  for (int location = 1 + side; location < 20; location += 2) {
    if (!miniboard[location]) {
      ...
      for (int i = 0; i < 4; i++) {
        // If this spot is taken by us and the other two are empty, this is a
        // win-in-two index
        if (winningPossibilitiesLocations[index][i][0] == -1) {
          break;
        }

        if (!miniboard[winningPossibilitiesLocations[index][i][0] +
                       posOffset] &&
            !miniboard[winningPossibilitiesLocations[index][i][0] +
                       sideOffset] &&
            !miniboard[winningPossibilitiesLocations[index][i][1] +
                       posOffset] &&
            !miniboard[winningPossibilitiesLocations[index][i][1] +
                       sideOffset]) {
          r++;
 ...
 */
significances calcSignificances(bitset<20> fullBoard[9], float evaluationsX[9],
                                float evaluationsY[9]) {
  ...
  for (int i = 0; i < 9; i++) {
    if (fullBoard[i][0] && fullBoard[i][1]) {
      ...
      for (int winIndex = 0; winIndex < 4; winIndex++) {
        ...
        // If either board is already won for the other side (or tied), sig is
        // zero
...
void Node::addChildren() {
  /**
   * Add all possible moves as children. Checks if children have already been
   * added and will not add again.
   */
  if (!hasChildren) {
...
float minimax(Node(&node), int depth, float alpha, float beta,
              bool maximizingPlayer, constants c) {
  return minimaxTimeLimited(node, depth, alpha, beta, maximizingPlayer, 0, c)
      .result;
};

/**
 * Calculates the evaluation of the given board to the given depth.
 * Note that updateMiniboardStatus() or updateSignleMiniboardStatus() must be
 * called before this function.
 */
timeLimitedSearchResult minimaxTimeLimited(Node(&node), int depth, float alpha,
                                           float beta, bool maximizingPlayer,
                                           int time, constants c) {
  ...
  for (Node i : node.children) {
    ...
    if (maximizingPlayer) {
      ...
      // If the position is lost, the best option is the one furthest from game
      // over
      ...
      // If the position is won, the best option is the one closest from game
      // over
...
GameState minimaxSearchTime(GameState position, int time, bool playAsX,
  ...
  while (true) {
    ...
    for (nodeAndEval &j : childEvals) {
      ...
      timeLimitedSearchResult result;
      result =
          minimaxTimeLimited(i, depth - 1, -1 * inf, inf, !playAsX, endTime, c);
...
int computerVcomputer(int depth1, constants c1, int depth2, constants c2,
                      bool displayGames) {
  GameState game;
  boardCoords move;
  ...
  while (game.getStatus() == 0) {
    ...
    if (displayGames) game.displayGame();
